<?php
    $page_title = $taxonomy->title ?? ($page->title ?? ($page->name ?? null));
    $image_background =
        $taxonomy->json_params->image_background ?? ($web_information->image->background_breadcrumbs ?? '');
?>

<?php $__env->startSection('content'); ?>
    <main id="fhm-product-detail" class="position-relative">
        <?php echo $__env->make('frontend.element.banner_home', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>

        <?php echo $__env->make('frontend.element.banner_product', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>

        <!-- START BREADCRUMB -->
        <section class="breadcrumb">
            <div class="container">
                <div class="breadcrumb-list">
                    <a href="/" title="Trang chủ" class="breadcrumb-link">Trang chủ</a>
                    <a href="<?php echo e(route('frontend.cms.product_all')); ?>" title="Sản phẩm" class="breadcrumb-link">Sản phẩm</a>
                    <a href="<?php echo e(route('frontend.cms.product', ['alias_detail' => $detail->url_part])); ?>"
                        title="<?php echo e($detail->title); ?>"
                        class="breadcrumb-link breadcrumb-link-current"><?php echo e($detail->title); ?></a>
                </div>
            </div>
        </section>
        <!-- END BREADCRUMB -->

        <!-- ============================================= -->

        <?php echo $__env->make('frontend.element.product_detail', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>

        <!-- ============================================= -->

        <?php echo $__env->make('frontend.element.tab_product', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
        <!-- ============================================= -->

        <!-- START PRODUCT DETAIL RELATED -->
        <section class="related bg-smoke">
            <h3>may you also like?</h3>
            <div class="swiper main-slider related-container">
                <div class="container">
                    <div class="related-slider main-slider-container">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide related-item">
                                <div class="product">
                                    <div class="product-img">
                                        <img class="product-img-default" src="./assets/image/product-1.png"
                                            alt="Culotte dress" title="Culotte dress" />
                                        <img class="product-img-hover" src="./assets/image/product-2.png"
                                            alt="Culotte dress" title="Culotte dress" />
                                        <div class="product-wishlist" data-bs-toggle="offcanvas"
                                            data-bs-target="#cart-popup" aria-controls="cart-popup">
                                            <svg width="17" height="15" viewBox="0 0 17 15" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M8.44597 14.6433C6.09973 12.9719 3.90258 11.1514 2.10955 8.89149C1.3327 7.91062 0.669627 6.86305 0.343978 5.63107C-0.342632 3.05334 1.32093 0.459919 3.96535 0.0832641C5.778 -0.175686 7.23362 0.507001 8.30081 1.99793C8.35181 2.06855 8.39497 2.13917 8.45774 2.23726C8.5009 2.17448 8.53621 2.12348 8.5676 2.0764C9.56417 0.526619 11.3337 -0.195303 12.9815 0.0871878C14.8609 0.412837 16.238 1.73113 16.6304 3.53201C16.8697 4.64235 16.6578 5.69385 16.187 6.70611C15.6573 7.84784 14.908 8.84048 14.0762 9.77427C12.4362 11.6065 10.5607 13.172 8.55191 14.5805C8.52052 14.5962 8.49305 14.6119 8.44597 14.6433ZM7.88099 4.45403C7.8653 4.31278 7.85353 4.19508 7.83783 4.0813C7.58281 1.71543 4.8756 0.432455 2.86678 1.7272C1.53672 2.58252 1.03843 4.18723 1.61519 5.75662C1.98399 6.76103 2.58821 7.62813 3.26305 8.44421C4.45971 9.8959 5.84863 11.1514 7.31601 12.3206C7.69266 12.6188 8.07717 12.9091 8.45774 13.2034C10.3371 11.7988 12.1027 10.3039 13.5975 8.51091C14.2213 7.76152 14.7824 6.96898 15.1747 6.0705C15.4729 5.38782 15.626 4.68551 15.5082 3.93613C15.2689 2.40989 13.9781 1.28385 12.3812 1.2093C10.9256 1.14261 9.52886 2.18625 9.16397 3.62617C9.09727 3.89297 9.06588 4.17154 9.0188 4.45403C8.64215 4.45403 8.26549 4.45403 7.88099 4.45403Z"
                                                    fill="#616161" />
                                            </svg>
                                        </div>
                                        <button class="main-btn main-btn-noborder product-quickview"
                                            data-bs-toggle="modal" data-bs-target="#fhm-quickview-popup"
                                            aria-controls="fhm-quickview-popup">
                                            Quick View
                                        </button>
                                    </div>
                                    <div class="product-info">
                                        <span class="product-type">Denim</span>
                                        <a href="#" class="product-name" title="Culotte dress">
                                            Culotte dress
                                        </a>
                                        <div class="product-price">
                                            <span class="product-price-current">$262.46</span>
                                            <span class="product-price-old">$292.99</span>
                                        </div>
                                    </div>
                                    <button class="main-btn add-cart" data-bs-toggle="offcanvas"
                                        data-bs-target="#cart-popup" aria-controls="cart-popup">
                                        <svg width="16" height="17" viewBox="0 0 16 17" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M1.72117 1.54131C1.33081 1.54131 0.949755 1.54596 0.568694 1.54131C0.159752 1.53666 -0.109779 1.14631 0.0435745 0.77919C0.131869 0.560777 0.303811 0.435306 0.540812 0.430659C1.10776 0.426012 1.67935 0.421365 2.24629 0.430659C2.53906 0.435306 2.75282 0.649072 2.78535 0.946485C2.8597 1.62496 2.92941 2.30808 2.99912 2.98655C3.00841 3.05626 3.0177 3.12596 3.03165 3.21426C3.106 3.21426 3.18035 3.21426 3.2547 3.21426C7.11643 3.21426 10.9781 3.21426 14.8399 3.21426C15.4347 3.21426 15.9041 3.56279 15.9877 4.11579C16.0202 4.33885 15.9831 4.57585 15.9366 4.79891C15.5137 6.79251 15.0908 8.78611 14.654 10.7797C14.4774 11.5976 13.8175 12.1181 12.981 12.1181C10.1184 12.1181 7.25119 12.1181 4.38859 12.1181C3.45918 12.1181 2.77141 11.4768 2.67847 10.5566C2.51582 8.9534 2.33923 7.35016 2.17194 5.75156C2.02788 4.38068 1.88382 3.00979 1.7444 1.63425C1.73511 1.61566 1.73046 1.59243 1.72117 1.54131ZM3.13853 4.3342C3.20823 4.97085 3.27329 5.58427 3.33835 6.20233C3.48706 7.61969 3.63577 9.03705 3.78912 10.4544C3.83094 10.8262 4.03541 11.0074 4.41647 11.0074C7.26513 11.0074 10.1091 11.0074 12.9578 11.0074C13.311 11.0074 13.5062 10.8494 13.5805 10.5009C13.9383 8.83258 14.2962 7.15963 14.654 5.49133C14.7376 5.11027 14.8166 4.72456 14.9003 4.3342C10.9735 4.3342 7.06531 4.3342 3.13853 4.3342Z"
                                                fill="white" />
                                            <path
                                                d="M3.34277 14.911C3.34277 13.9909 4.09095 13.238 5.00643 13.238C5.92655 13.238 6.67938 13.9862 6.67938 14.9017C6.67938 15.8218 5.9312 16.5746 5.01572 16.5746C4.10025 16.5793 3.34277 15.8265 3.34277 14.911ZM5.00643 15.4593C5.30849 15.4593 5.56408 15.213 5.56408 14.911C5.56872 14.6089 5.32243 14.3533 5.02037 14.3487C4.71366 14.344 4.45342 14.5996 4.45342 14.9063C4.45807 15.2084 4.70901 15.4593 5.00643 15.4593Z"
                                                fill="white" />
                                            <path
                                                d="M11.6893 16.5746C10.7738 16.5746 10.021 15.8218 10.021 14.9017C10.021 13.9862 10.7738 13.2334 11.6939 13.2334C12.6141 13.2334 13.3622 13.9862 13.3622 14.9063C13.3622 15.8265 12.6094 16.5746 11.6893 16.5746ZM11.6893 14.3487C11.3872 14.3487 11.1363 14.5996 11.1363 14.9017C11.1363 15.2038 11.3826 15.4547 11.6846 15.4593C11.9914 15.464 12.2516 15.2038 12.2469 14.8971C12.2469 14.595 11.9914 14.3487 11.6893 14.3487Z"
                                                fill="white" />
                                        </svg>

                                        Add to Cart
                                    </button>
                                </div>
                            </div>
                            <div class="swiper-slide related-item">
                                <div class="product">
                                    <div class="product-img">
                                        <img class="product-img-default" src="./assets/image/product-2.png"
                                            alt="Culotte dress" title="Culotte dress" />
                                        <img class="product-img-hover" src="./assets/image/product-3.png"
                                            alt="Culotte dress" title="Culotte dress" />
                                        <div class="product-wishlist" data-bs-toggle="offcanvas"
                                            data-bs-target="#cart-popup" aria-controls="cart-popup">
                                            <svg width="17" height="15" viewBox="0 0 17 15" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M8.44597 14.6433C6.09973 12.9719 3.90258 11.1514 2.10955 8.89149C1.3327 7.91062 0.669627 6.86305 0.343978 5.63107C-0.342632 3.05334 1.32093 0.459919 3.96535 0.0832641C5.778 -0.175686 7.23362 0.507001 8.30081 1.99793C8.35181 2.06855 8.39497 2.13917 8.45774 2.23726C8.5009 2.17448 8.53621 2.12348 8.5676 2.0764C9.56417 0.526619 11.3337 -0.195303 12.9815 0.0871878C14.8609 0.412837 16.238 1.73113 16.6304 3.53201C16.8697 4.64235 16.6578 5.69385 16.187 6.70611C15.6573 7.84784 14.908 8.84048 14.0762 9.77427C12.4362 11.6065 10.5607 13.172 8.55191 14.5805C8.52052 14.5962 8.49305 14.6119 8.44597 14.6433ZM7.88099 4.45403C7.8653 4.31278 7.85353 4.19508 7.83783 4.0813C7.58281 1.71543 4.8756 0.432455 2.86678 1.7272C1.53672 2.58252 1.03843 4.18723 1.61519 5.75662C1.98399 6.76103 2.58821 7.62813 3.26305 8.44421C4.45971 9.8959 5.84863 11.1514 7.31601 12.3206C7.69266 12.6188 8.07717 12.9091 8.45774 13.2034C10.3371 11.7988 12.1027 10.3039 13.5975 8.51091C14.2213 7.76152 14.7824 6.96898 15.1747 6.0705C15.4729 5.38782 15.626 4.68551 15.5082 3.93613C15.2689 2.40989 13.9781 1.28385 12.3812 1.2093C10.9256 1.14261 9.52886 2.18625 9.16397 3.62617C9.09727 3.89297 9.06588 4.17154 9.0188 4.45403C8.64215 4.45403 8.26549 4.45403 7.88099 4.45403Z"
                                                    fill="#616161" />
                                            </svg>
                                        </div>
                                        <button class="main-btn main-btn-noborder product-quickview"
                                            data-bs-toggle="modal" data-bs-target="#fhm-quickview-popup"
                                            aria-controls="fhm-quickview-popup">
                                            Quick View
                                        </button>
                                    </div>
                                    <div class="product-info">
                                        <span class="product-type">Dress</span>
                                        <a href="#" class="product-name" title="Culotte dress">
                                            Wrap around dress
                                        </a>
                                        <div class="product-price">
                                            <span class="product-price-current">$134.46</span>
                                            <span class="product-price-old">$292.99</span>
                                        </div>
                                    </div>
                                    <button class="main-btn add-cart" data-bs-toggle="offcanvas"
                                        data-bs-target="#cart-popup" aria-controls="cart-popup">
                                        <svg width="16" height="17" viewBox="0 0 16 17" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M1.72117 1.54131C1.33081 1.54131 0.949755 1.54596 0.568694 1.54131C0.159752 1.53666 -0.109779 1.14631 0.0435745 0.77919C0.131869 0.560777 0.303811 0.435306 0.540812 0.430659C1.10776 0.426012 1.67935 0.421365 2.24629 0.430659C2.53906 0.435306 2.75282 0.649072 2.78535 0.946485C2.8597 1.62496 2.92941 2.30808 2.99912 2.98655C3.00841 3.05626 3.0177 3.12596 3.03165 3.21426C3.106 3.21426 3.18035 3.21426 3.2547 3.21426C7.11643 3.21426 10.9781 3.21426 14.8399 3.21426C15.4347 3.21426 15.9041 3.56279 15.9877 4.11579C16.0202 4.33885 15.9831 4.57585 15.9366 4.79891C15.5137 6.79251 15.0908 8.78611 14.654 10.7797C14.4774 11.5976 13.8175 12.1181 12.981 12.1181C10.1184 12.1181 7.25119 12.1181 4.38859 12.1181C3.45918 12.1181 2.77141 11.4768 2.67847 10.5566C2.51582 8.9534 2.33923 7.35016 2.17194 5.75156C2.02788 4.38068 1.88382 3.00979 1.7444 1.63425C1.73511 1.61566 1.73046 1.59243 1.72117 1.54131ZM3.13853 4.3342C3.20823 4.97085 3.27329 5.58427 3.33835 6.20233C3.48706 7.61969 3.63577 9.03705 3.78912 10.4544C3.83094 10.8262 4.03541 11.0074 4.41647 11.0074C7.26513 11.0074 10.1091 11.0074 12.9578 11.0074C13.311 11.0074 13.5062 10.8494 13.5805 10.5009C13.9383 8.83258 14.2962 7.15963 14.654 5.49133C14.7376 5.11027 14.8166 4.72456 14.9003 4.3342C10.9735 4.3342 7.06531 4.3342 3.13853 4.3342Z"
                                                fill="white" />
                                            <path
                                                d="M3.34277 14.911C3.34277 13.9909 4.09095 13.238 5.00643 13.238C5.92655 13.238 6.67938 13.9862 6.67938 14.9017C6.67938 15.8218 5.9312 16.5746 5.01572 16.5746C4.10025 16.5793 3.34277 15.8265 3.34277 14.911ZM5.00643 15.4593C5.30849 15.4593 5.56408 15.213 5.56408 14.911C5.56872 14.6089 5.32243 14.3533 5.02037 14.3487C4.71366 14.344 4.45342 14.5996 4.45342 14.9063C4.45807 15.2084 4.70901 15.4593 5.00643 15.4593Z"
                                                fill="white" />
                                            <path
                                                d="M11.6893 16.5746C10.7738 16.5746 10.021 15.8218 10.021 14.9017C10.021 13.9862 10.7738 13.2334 11.6939 13.2334C12.6141 13.2334 13.3622 13.9862 13.3622 14.9063C13.3622 15.8265 12.6094 16.5746 11.6893 16.5746ZM11.6893 14.3487C11.3872 14.3487 11.1363 14.5996 11.1363 14.9017C11.1363 15.2038 11.3826 15.4547 11.6846 15.4593C11.9914 15.464 12.2516 15.2038 12.2469 14.8971C12.2469 14.595 11.9914 14.3487 11.6893 14.3487Z"
                                                fill="white" />
                                        </svg>

                                        Add to Cart
                                    </button>
                                </div>
                            </div>
                            <div class="swiper-slide related-item">
                                <div class="product">
                                    <div class="product-img">
                                        <img class="product-img-default" src="./assets/image/product-3.png"
                                            alt="Culotte dress" title="Culotte dress" />
                                        <img class="product-img-hover" src="./assets/image/product-4.png"
                                            alt="Culotte dress" title="Culotte dress" />
                                        <div class="product-wishlist" data-bs-toggle="offcanvas"
                                            data-bs-target="#cart-popup" aria-controls="cart-popup">
                                            <svg width="17" height="15" viewBox="0 0 17 15" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M8.44597 14.6433C6.09973 12.9719 3.90258 11.1514 2.10955 8.89149C1.3327 7.91062 0.669627 6.86305 0.343978 5.63107C-0.342632 3.05334 1.32093 0.459919 3.96535 0.0832641C5.778 -0.175686 7.23362 0.507001 8.30081 1.99793C8.35181 2.06855 8.39497 2.13917 8.45774 2.23726C8.5009 2.17448 8.53621 2.12348 8.5676 2.0764C9.56417 0.526619 11.3337 -0.195303 12.9815 0.0871878C14.8609 0.412837 16.238 1.73113 16.6304 3.53201C16.8697 4.64235 16.6578 5.69385 16.187 6.70611C15.6573 7.84784 14.908 8.84048 14.0762 9.77427C12.4362 11.6065 10.5607 13.172 8.55191 14.5805C8.52052 14.5962 8.49305 14.6119 8.44597 14.6433ZM7.88099 4.45403C7.8653 4.31278 7.85353 4.19508 7.83783 4.0813C7.58281 1.71543 4.8756 0.432455 2.86678 1.7272C1.53672 2.58252 1.03843 4.18723 1.61519 5.75662C1.98399 6.76103 2.58821 7.62813 3.26305 8.44421C4.45971 9.8959 5.84863 11.1514 7.31601 12.3206C7.69266 12.6188 8.07717 12.9091 8.45774 13.2034C10.3371 11.7988 12.1027 10.3039 13.5975 8.51091C14.2213 7.76152 14.7824 6.96898 15.1747 6.0705C15.4729 5.38782 15.626 4.68551 15.5082 3.93613C15.2689 2.40989 13.9781 1.28385 12.3812 1.2093C10.9256 1.14261 9.52886 2.18625 9.16397 3.62617C9.09727 3.89297 9.06588 4.17154 9.0188 4.45403C8.64215 4.45403 8.26549 4.45403 7.88099 4.45403Z"
                                                    fill="#616161" />
                                            </svg>
                                        </div>
                                        <button class="main-btn main-btn-noborder product-quickview"
                                            data-bs-toggle="modal" data-bs-target="#fhm-quickview-popup"
                                            aria-controls="fhm-quickview-popup">
                                            Quick View
                                        </button>
                                    </div>
                                    <div class="product-info">
                                        <span class="product-type">Denim</span>
                                        <a href="#" class="product-name" title="Culotte dress">
                                            Bubble dress
                                        </a>
                                        <div class="product-price">
                                            <span class="product-price-current">$219.46</span>
                                            <span class="product-price-old">$292.99</span>
                                        </div>
                                    </div>
                                    <button class="main-btn add-cart" data-bs-toggle="offcanvas"
                                        data-bs-target="#cart-popup" aria-controls="cart-popup">
                                        <svg width="16" height="17" viewBox="0 0 16 17" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M1.72117 1.54131C1.33081 1.54131 0.949755 1.54596 0.568694 1.54131C0.159752 1.53666 -0.109779 1.14631 0.0435745 0.77919C0.131869 0.560777 0.303811 0.435306 0.540812 0.430659C1.10776 0.426012 1.67935 0.421365 2.24629 0.430659C2.53906 0.435306 2.75282 0.649072 2.78535 0.946485C2.8597 1.62496 2.92941 2.30808 2.99912 2.98655C3.00841 3.05626 3.0177 3.12596 3.03165 3.21426C3.106 3.21426 3.18035 3.21426 3.2547 3.21426C7.11643 3.21426 10.9781 3.21426 14.8399 3.21426C15.4347 3.21426 15.9041 3.56279 15.9877 4.11579C16.0202 4.33885 15.9831 4.57585 15.9366 4.79891C15.5137 6.79251 15.0908 8.78611 14.654 10.7797C14.4774 11.5976 13.8175 12.1181 12.981 12.1181C10.1184 12.1181 7.25119 12.1181 4.38859 12.1181C3.45918 12.1181 2.77141 11.4768 2.67847 10.5566C2.51582 8.9534 2.33923 7.35016 2.17194 5.75156C2.02788 4.38068 1.88382 3.00979 1.7444 1.63425C1.73511 1.61566 1.73046 1.59243 1.72117 1.54131ZM3.13853 4.3342C3.20823 4.97085 3.27329 5.58427 3.33835 6.20233C3.48706 7.61969 3.63577 9.03705 3.78912 10.4544C3.83094 10.8262 4.03541 11.0074 4.41647 11.0074C7.26513 11.0074 10.1091 11.0074 12.9578 11.0074C13.311 11.0074 13.5062 10.8494 13.5805 10.5009C13.9383 8.83258 14.2962 7.15963 14.654 5.49133C14.7376 5.11027 14.8166 4.72456 14.9003 4.3342C10.9735 4.3342 7.06531 4.3342 3.13853 4.3342Z"
                                                fill="white" />
                                            <path
                                                d="M3.34277 14.911C3.34277 13.9909 4.09095 13.238 5.00643 13.238C5.92655 13.238 6.67938 13.9862 6.67938 14.9017C6.67938 15.8218 5.9312 16.5746 5.01572 16.5746C4.10025 16.5793 3.34277 15.8265 3.34277 14.911ZM5.00643 15.4593C5.30849 15.4593 5.56408 15.213 5.56408 14.911C5.56872 14.6089 5.32243 14.3533 5.02037 14.3487C4.71366 14.344 4.45342 14.5996 4.45342 14.9063C4.45807 15.2084 4.70901 15.4593 5.00643 15.4593Z"
                                                fill="white" />
                                            <path
                                                d="M11.6893 16.5746C10.7738 16.5746 10.021 15.8218 10.021 14.9017C10.021 13.9862 10.7738 13.2334 11.6939 13.2334C12.6141 13.2334 13.3622 13.9862 13.3622 14.9063C13.3622 15.8265 12.6094 16.5746 11.6893 16.5746ZM11.6893 14.3487C11.3872 14.3487 11.1363 14.5996 11.1363 14.9017C11.1363 15.2038 11.3826 15.4547 11.6846 15.4593C11.9914 15.464 12.2516 15.2038 12.2469 14.8971C12.2469 14.595 11.9914 14.3487 11.6893 14.3487Z"
                                                fill="white" />
                                        </svg>

                                        Add to Cart
                                    </button>
                                </div>
                            </div>
                            <div class="swiper-slide related-item">
                                <div class="product">
                                    <div class="product-img">
                                        <img class="product-img-default" src="./assets/image/product-4.png"
                                            alt="Culotte dress" title="Culotte dress" />
                                        <img class="product-img-hover" src="./assets/image/product-5.png"
                                            alt="Culotte dress" title="Culotte dress" />
                                        <div class="product-wishlist" data-bs-toggle="offcanvas"
                                            data-bs-target="#cart-popup" aria-controls="cart-popup">
                                            <svg width="17" height="15" viewBox="0 0 17 15" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M8.44597 14.6433C6.09973 12.9719 3.90258 11.1514 2.10955 8.89149C1.3327 7.91062 0.669627 6.86305 0.343978 5.63107C-0.342632 3.05334 1.32093 0.459919 3.96535 0.0832641C5.778 -0.175686 7.23362 0.507001 8.30081 1.99793C8.35181 2.06855 8.39497 2.13917 8.45774 2.23726C8.5009 2.17448 8.53621 2.12348 8.5676 2.0764C9.56417 0.526619 11.3337 -0.195303 12.9815 0.0871878C14.8609 0.412837 16.238 1.73113 16.6304 3.53201C16.8697 4.64235 16.6578 5.69385 16.187 6.70611C15.6573 7.84784 14.908 8.84048 14.0762 9.77427C12.4362 11.6065 10.5607 13.172 8.55191 14.5805C8.52052 14.5962 8.49305 14.6119 8.44597 14.6433ZM7.88099 4.45403C7.8653 4.31278 7.85353 4.19508 7.83783 4.0813C7.58281 1.71543 4.8756 0.432455 2.86678 1.7272C1.53672 2.58252 1.03843 4.18723 1.61519 5.75662C1.98399 6.76103 2.58821 7.62813 3.26305 8.44421C4.45971 9.8959 5.84863 11.1514 7.31601 12.3206C7.69266 12.6188 8.07717 12.9091 8.45774 13.2034C10.3371 11.7988 12.1027 10.3039 13.5975 8.51091C14.2213 7.76152 14.7824 6.96898 15.1747 6.0705C15.4729 5.38782 15.626 4.68551 15.5082 3.93613C15.2689 2.40989 13.9781 1.28385 12.3812 1.2093C10.9256 1.14261 9.52886 2.18625 9.16397 3.62617C9.09727 3.89297 9.06588 4.17154 9.0188 4.45403C8.64215 4.45403 8.26549 4.45403 7.88099 4.45403Z"
                                                    fill="#616161" />
                                            </svg>
                                        </div>
                                        <button class="main-btn main-btn-noborder product-quickview"
                                            data-bs-toggle="modal" data-bs-target="#fhm-quickview-popup"
                                            aria-controls="fhm-quickview-popup">
                                            Quick View
                                        </button>
                                    </div>
                                    <div class="product-info">
                                        <span class="product-type">Dress</span>
                                        <a href="#" class="product-name" title="Culotte dress">
                                            MIMI Dress
                                        </a>
                                        <div class="product-price">
                                            <span class="product-price-current">$198.46</span>
                                            <span class="product-price-old">$292.99</span>
                                        </div>
                                    </div>
                                    <button class="main-btn add-cart" data-bs-toggle="offcanvas"
                                        data-bs-target="#cart-popup" aria-controls="cart-popup">
                                        <svg width="16" height="17" viewBox="0 0 16 17" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M1.72117 1.54131C1.33081 1.54131 0.949755 1.54596 0.568694 1.54131C0.159752 1.53666 -0.109779 1.14631 0.0435745 0.77919C0.131869 0.560777 0.303811 0.435306 0.540812 0.430659C1.10776 0.426012 1.67935 0.421365 2.24629 0.430659C2.53906 0.435306 2.75282 0.649072 2.78535 0.946485C2.8597 1.62496 2.92941 2.30808 2.99912 2.98655C3.00841 3.05626 3.0177 3.12596 3.03165 3.21426C3.106 3.21426 3.18035 3.21426 3.2547 3.21426C7.11643 3.21426 10.9781 3.21426 14.8399 3.21426C15.4347 3.21426 15.9041 3.56279 15.9877 4.11579C16.0202 4.33885 15.9831 4.57585 15.9366 4.79891C15.5137 6.79251 15.0908 8.78611 14.654 10.7797C14.4774 11.5976 13.8175 12.1181 12.981 12.1181C10.1184 12.1181 7.25119 12.1181 4.38859 12.1181C3.45918 12.1181 2.77141 11.4768 2.67847 10.5566C2.51582 8.9534 2.33923 7.35016 2.17194 5.75156C2.02788 4.38068 1.88382 3.00979 1.7444 1.63425C1.73511 1.61566 1.73046 1.59243 1.72117 1.54131ZM3.13853 4.3342C3.20823 4.97085 3.27329 5.58427 3.33835 6.20233C3.48706 7.61969 3.63577 9.03705 3.78912 10.4544C3.83094 10.8262 4.03541 11.0074 4.41647 11.0074C7.26513 11.0074 10.1091 11.0074 12.9578 11.0074C13.311 11.0074 13.5062 10.8494 13.5805 10.5009C13.9383 8.83258 14.2962 7.15963 14.654 5.49133C14.7376 5.11027 14.8166 4.72456 14.9003 4.3342C10.9735 4.3342 7.06531 4.3342 3.13853 4.3342Z"
                                                fill="white" />
                                            <path
                                                d="M3.34277 14.911C3.34277 13.9909 4.09095 13.238 5.00643 13.238C5.92655 13.238 6.67938 13.9862 6.67938 14.9017C6.67938 15.8218 5.9312 16.5746 5.01572 16.5746C4.10025 16.5793 3.34277 15.8265 3.34277 14.911ZM5.00643 15.4593C5.30849 15.4593 5.56408 15.213 5.56408 14.911C5.56872 14.6089 5.32243 14.3533 5.02037 14.3487C4.71366 14.344 4.45342 14.5996 4.45342 14.9063C4.45807 15.2084 4.70901 15.4593 5.00643 15.4593Z"
                                                fill="white" />
                                            <path
                                                d="M11.6893 16.5746C10.7738 16.5746 10.021 15.8218 10.021 14.9017C10.021 13.9862 10.7738 13.2334 11.6939 13.2334C12.6141 13.2334 13.3622 13.9862 13.3622 14.9063C13.3622 15.8265 12.6094 16.5746 11.6893 16.5746ZM11.6893 14.3487C11.3872 14.3487 11.1363 14.5996 11.1363 14.9017C11.1363 15.2038 11.3826 15.4547 11.6846 15.4593C11.9914 15.464 12.2516 15.2038 12.2469 14.8971C12.2469 14.595 11.9914 14.3487 11.6893 14.3487Z"
                                                fill="white" />
                                        </svg>

                                        Add to Cart
                                    </button>
                                </div>
                            </div>
                            <div class="swiper-slide related-item">
                                <div class="product">
                                    <div class="product-img">
                                        <img class="product-img-default" src="./assets/image/product-5.png"
                                            alt="Culotte dress" title="Culotte dress" />
                                        <img class="product-img-hover" src="./assets/image/product-6.png"
                                            alt="Culotte dress" title="Culotte dress" />
                                        <div class="product-wishlist" data-bs-toggle="offcanvas"
                                            data-bs-target="#cart-popup" aria-controls="cart-popup">
                                            <svg width="17" height="15" viewBox="0 0 17 15" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M8.44597 14.6433C6.09973 12.9719 3.90258 11.1514 2.10955 8.89149C1.3327 7.91062 0.669627 6.86305 0.343978 5.63107C-0.342632 3.05334 1.32093 0.459919 3.96535 0.0832641C5.778 -0.175686 7.23362 0.507001 8.30081 1.99793C8.35181 2.06855 8.39497 2.13917 8.45774 2.23726C8.5009 2.17448 8.53621 2.12348 8.5676 2.0764C9.56417 0.526619 11.3337 -0.195303 12.9815 0.0871878C14.8609 0.412837 16.238 1.73113 16.6304 3.53201C16.8697 4.64235 16.6578 5.69385 16.187 6.70611C15.6573 7.84784 14.908 8.84048 14.0762 9.77427C12.4362 11.6065 10.5607 13.172 8.55191 14.5805C8.52052 14.5962 8.49305 14.6119 8.44597 14.6433ZM7.88099 4.45403C7.8653 4.31278 7.85353 4.19508 7.83783 4.0813C7.58281 1.71543 4.8756 0.432455 2.86678 1.7272C1.53672 2.58252 1.03843 4.18723 1.61519 5.75662C1.98399 6.76103 2.58821 7.62813 3.26305 8.44421C4.45971 9.8959 5.84863 11.1514 7.31601 12.3206C7.69266 12.6188 8.07717 12.9091 8.45774 13.2034C10.3371 11.7988 12.1027 10.3039 13.5975 8.51091C14.2213 7.76152 14.7824 6.96898 15.1747 6.0705C15.4729 5.38782 15.626 4.68551 15.5082 3.93613C15.2689 2.40989 13.9781 1.28385 12.3812 1.2093C10.9256 1.14261 9.52886 2.18625 9.16397 3.62617C9.09727 3.89297 9.06588 4.17154 9.0188 4.45403C8.64215 4.45403 8.26549 4.45403 7.88099 4.45403Z"
                                                    fill="#616161" />
                                            </svg>
                                        </div>
                                        <button class="main-btn main-btn-noborder product-quickview"
                                            data-bs-toggle="modal" data-bs-target="#fhm-quickview-popup"
                                            aria-controls="fhm-quickview-popup">
                                            Quick View
                                        </button>
                                    </div>
                                    <div class="product-info">
                                        <span class="product-type">Denim</span>
                                        <a href="#" class="product-name" title="Culotte dress">
                                            Culotte dress
                                        </a>
                                        <div class="product-price">
                                            <span class="product-price-current">$262.46</span>
                                            <span class="product-price-old">$292.99</span>
                                        </div>
                                    </div>
                                    <button class="main-btn add-cart" data-bs-toggle="offcanvas"
                                        data-bs-target="#cart-popup" aria-controls="cart-popup">
                                        <svg width="16" height="17" viewBox="0 0 16 17" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M1.72117 1.54131C1.33081 1.54131 0.949755 1.54596 0.568694 1.54131C0.159752 1.53666 -0.109779 1.14631 0.0435745 0.77919C0.131869 0.560777 0.303811 0.435306 0.540812 0.430659C1.10776 0.426012 1.67935 0.421365 2.24629 0.430659C2.53906 0.435306 2.75282 0.649072 2.78535 0.946485C2.8597 1.62496 2.92941 2.30808 2.99912 2.98655C3.00841 3.05626 3.0177 3.12596 3.03165 3.21426C3.106 3.21426 3.18035 3.21426 3.2547 3.21426C7.11643 3.21426 10.9781 3.21426 14.8399 3.21426C15.4347 3.21426 15.9041 3.56279 15.9877 4.11579C16.0202 4.33885 15.9831 4.57585 15.9366 4.79891C15.5137 6.79251 15.0908 8.78611 14.654 10.7797C14.4774 11.5976 13.8175 12.1181 12.981 12.1181C10.1184 12.1181 7.25119 12.1181 4.38859 12.1181C3.45918 12.1181 2.77141 11.4768 2.67847 10.5566C2.51582 8.9534 2.33923 7.35016 2.17194 5.75156C2.02788 4.38068 1.88382 3.00979 1.7444 1.63425C1.73511 1.61566 1.73046 1.59243 1.72117 1.54131ZM3.13853 4.3342C3.20823 4.97085 3.27329 5.58427 3.33835 6.20233C3.48706 7.61969 3.63577 9.03705 3.78912 10.4544C3.83094 10.8262 4.03541 11.0074 4.41647 11.0074C7.26513 11.0074 10.1091 11.0074 12.9578 11.0074C13.311 11.0074 13.5062 10.8494 13.5805 10.5009C13.9383 8.83258 14.2962 7.15963 14.654 5.49133C14.7376 5.11027 14.8166 4.72456 14.9003 4.3342C10.9735 4.3342 7.06531 4.3342 3.13853 4.3342Z"
                                                fill="white" />
                                            <path
                                                d="M3.34277 14.911C3.34277 13.9909 4.09095 13.238 5.00643 13.238C5.92655 13.238 6.67938 13.9862 6.67938 14.9017C6.67938 15.8218 5.9312 16.5746 5.01572 16.5746C4.10025 16.5793 3.34277 15.8265 3.34277 14.911ZM5.00643 15.4593C5.30849 15.4593 5.56408 15.213 5.56408 14.911C5.56872 14.6089 5.32243 14.3533 5.02037 14.3487C4.71366 14.344 4.45342 14.5996 4.45342 14.9063C4.45807 15.2084 4.70901 15.4593 5.00643 15.4593Z"
                                                fill="white" />
                                            <path
                                                d="M11.6893 16.5746C10.7738 16.5746 10.021 15.8218 10.021 14.9017C10.021 13.9862 10.7738 13.2334 11.6939 13.2334C12.6141 13.2334 13.3622 13.9862 13.3622 14.9063C13.3622 15.8265 12.6094 16.5746 11.6893 16.5746ZM11.6893 14.3487C11.3872 14.3487 11.1363 14.5996 11.1363 14.9017C11.1363 15.2038 11.3826 15.4547 11.6846 15.4593C11.9914 15.464 12.2516 15.2038 12.2469 14.8971C12.2469 14.595 11.9914 14.3487 11.6893 14.3487Z"
                                                fill="white" />
                                        </svg>

                                        Add to Cart
                                    </button>
                                </div>
                            </div>
                            <div class="swiper-slide related-item">
                                <div class="product">
                                    <div class="product-img">
                                        <img class="product-img-default" src="./assets/image/product-6.png"
                                            alt="Culotte dress" title="Culotte dress" />
                                        <img class="product-img-hover" src="./assets/image/product-1.png"
                                            alt="Culotte dress" title="Culotte dress" />
                                        <div class="product-wishlist" data-bs-toggle="offcanvas"
                                            data-bs-target="#cart-popup" aria-controls="cart-popup">
                                            <svg width="17" height="15" viewBox="0 0 17 15" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M8.44597 14.6433C6.09973 12.9719 3.90258 11.1514 2.10955 8.89149C1.3327 7.91062 0.669627 6.86305 0.343978 5.63107C-0.342632 3.05334 1.32093 0.459919 3.96535 0.0832641C5.778 -0.175686 7.23362 0.507001 8.30081 1.99793C8.35181 2.06855 8.39497 2.13917 8.45774 2.23726C8.5009 2.17448 8.53621 2.12348 8.5676 2.0764C9.56417 0.526619 11.3337 -0.195303 12.9815 0.0871878C14.8609 0.412837 16.238 1.73113 16.6304 3.53201C16.8697 4.64235 16.6578 5.69385 16.187 6.70611C15.6573 7.84784 14.908 8.84048 14.0762 9.77427C12.4362 11.6065 10.5607 13.172 8.55191 14.5805C8.52052 14.5962 8.49305 14.6119 8.44597 14.6433ZM7.88099 4.45403C7.8653 4.31278 7.85353 4.19508 7.83783 4.0813C7.58281 1.71543 4.8756 0.432455 2.86678 1.7272C1.53672 2.58252 1.03843 4.18723 1.61519 5.75662C1.98399 6.76103 2.58821 7.62813 3.26305 8.44421C4.45971 9.8959 5.84863 11.1514 7.31601 12.3206C7.69266 12.6188 8.07717 12.9091 8.45774 13.2034C10.3371 11.7988 12.1027 10.3039 13.5975 8.51091C14.2213 7.76152 14.7824 6.96898 15.1747 6.0705C15.4729 5.38782 15.626 4.68551 15.5082 3.93613C15.2689 2.40989 13.9781 1.28385 12.3812 1.2093C10.9256 1.14261 9.52886 2.18625 9.16397 3.62617C9.09727 3.89297 9.06588 4.17154 9.0188 4.45403C8.64215 4.45403 8.26549 4.45403 7.88099 4.45403Z"
                                                    fill="#616161" />
                                            </svg>
                                        </div>
                                        <button class="main-btn main-btn-noborder product-quickview"
                                            data-bs-toggle="modal" data-bs-target="#fhm-quickview-popup"
                                            aria-controls="fhm-quickview-popup">
                                            Quick View
                                        </button>
                                    </div>
                                    <div class="product-info">
                                        <span class="product-type">Dress</span>
                                        <a href="#" class="product-name" title="Culotte dress">
                                            MIMI Dress
                                        </a>
                                        <div class="product-price">
                                            <span class="product-price-current">$198.46</span>
                                            <span class="product-price-old">$292.99</span>
                                        </div>
                                    </div>
                                    <button class="main-btn add-cart" data-bs-toggle="offcanvas"
                                        data-bs-target="#cart-popup" aria-controls="cart-popup">
                                        <svg width="16" height="17" viewBox="0 0 16 17" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M1.72117 1.54131C1.33081 1.54131 0.949755 1.54596 0.568694 1.54131C0.159752 1.53666 -0.109779 1.14631 0.0435745 0.77919C0.131869 0.560777 0.303811 0.435306 0.540812 0.430659C1.10776 0.426012 1.67935 0.421365 2.24629 0.430659C2.53906 0.435306 2.75282 0.649072 2.78535 0.946485C2.8597 1.62496 2.92941 2.30808 2.99912 2.98655C3.00841 3.05626 3.0177 3.12596 3.03165 3.21426C3.106 3.21426 3.18035 3.21426 3.2547 3.21426C7.11643 3.21426 10.9781 3.21426 14.8399 3.21426C15.4347 3.21426 15.9041 3.56279 15.9877 4.11579C16.0202 4.33885 15.9831 4.57585 15.9366 4.79891C15.5137 6.79251 15.0908 8.78611 14.654 10.7797C14.4774 11.5976 13.8175 12.1181 12.981 12.1181C10.1184 12.1181 7.25119 12.1181 4.38859 12.1181C3.45918 12.1181 2.77141 11.4768 2.67847 10.5566C2.51582 8.9534 2.33923 7.35016 2.17194 5.75156C2.02788 4.38068 1.88382 3.00979 1.7444 1.63425C1.73511 1.61566 1.73046 1.59243 1.72117 1.54131ZM3.13853 4.3342C3.20823 4.97085 3.27329 5.58427 3.33835 6.20233C3.48706 7.61969 3.63577 9.03705 3.78912 10.4544C3.83094 10.8262 4.03541 11.0074 4.41647 11.0074C7.26513 11.0074 10.1091 11.0074 12.9578 11.0074C13.311 11.0074 13.5062 10.8494 13.5805 10.5009C13.9383 8.83258 14.2962 7.15963 14.654 5.49133C14.7376 5.11027 14.8166 4.72456 14.9003 4.3342C10.9735 4.3342 7.06531 4.3342 3.13853 4.3342Z"
                                                fill="white" />
                                            <path
                                                d="M3.34277 14.911C3.34277 13.9909 4.09095 13.238 5.00643 13.238C5.92655 13.238 6.67938 13.9862 6.67938 14.9017C6.67938 15.8218 5.9312 16.5746 5.01572 16.5746C4.10025 16.5793 3.34277 15.8265 3.34277 14.911ZM5.00643 15.4593C5.30849 15.4593 5.56408 15.213 5.56408 14.911C5.56872 14.6089 5.32243 14.3533 5.02037 14.3487C4.71366 14.344 4.45342 14.5996 4.45342 14.9063C4.45807 15.2084 4.70901 15.4593 5.00643 15.4593Z"
                                                fill="white" />
                                            <path
                                                d="M11.6893 16.5746C10.7738 16.5746 10.021 15.8218 10.021 14.9017C10.021 13.9862 10.7738 13.2334 11.6939 13.2334C12.6141 13.2334 13.3622 13.9862 13.3622 14.9063C13.3622 15.8265 12.6094 16.5746 11.6893 16.5746ZM11.6893 14.3487C11.3872 14.3487 11.1363 14.5996 11.1363 14.9017C11.1363 15.2038 11.3826 15.4547 11.6846 15.4593C11.9914 15.464 12.2516 15.2038 12.2469 14.8971C12.2469 14.595 11.9914 14.3487 11.6893 14.3487Z"
                                                fill="white" />
                                        </svg>

                                        Add to Cart
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="swiper-button-prev main-slider-prev related-prev">
                    <svg width="11" height="16" viewBox="0 0 11 16" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M0.34363 6.98486L6.98426 0.344238C7.44324 -0.114746 8.18543 -0.114746 8.63953 0.344238L9.74304 1.44775C10.202 1.90674 10.202 2.64893 9.74304 3.10303L5.0409 7.81494L9.74793 12.522C10.2069 12.981 10.2069 13.7231 9.74793 14.1772L8.64441 15.2856C8.18543 15.7446 7.44324 15.7446 6.98914 15.2856L0.348513 8.64502C-0.115354 8.18604 -0.115354 7.44385 0.34363 6.98486Z"
                            fill="#969595" />
                    </svg>
                </div>
                <div class="swiper-button-next main-slider-next related-next">
                    <svg width="11" height="16" viewBox="0 0 11 16" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M9.74865 6.98486L3.10803 0.344238C2.64905 -0.114746 1.90686 -0.114746 1.45276 0.344238L0.34924 1.44775C-0.109744 1.90674 -0.109744 2.64893 0.34924 3.10303L5.05139 7.81494L0.344357 12.522C-0.114627 12.981 -0.114627 13.7231 0.344357 14.1772L1.44787 15.2856C1.90686 15.7446 2.64904 15.7446 3.10315 15.2856L9.74377 8.64502C10.2076 8.18604 10.2076 7.44385 9.74865 6.98486Z"
                            fill="#969595" />
                    </svg>
                </div>
            </div>
        </section>
        <!-- END PRODUCT DETAIL RELATED -->

        <!-- ============================================= -->

        <!-- START PRODUCT DETAIL REVIEW -->
        <section class="review">
            <div class="container">
                <div class="review-heading">
                    <h5>Reviews</h5>
                    <span>(0)</span>
                </div>
                <div class="review-container">
                    <a href="#review-comments" class="main-btn review-btn" title="Write a Review">Write a Review</a>
                    <div class="review-content">
                        <div class="review-star">
                            <h2>0</h2>
                            <div class="review-rating" style="--rating: 0">
                                <img src="./assets/image/rating.svg" alt="rating" />
                            </div>
                            <h6>0 review</h6>
                        </div>
                        <div class="review-rate">
                            <div class="review-rate-item">
                                <span>5</span>
                                <span class="review-rate-line" style="--rating: 90%"></span>
                                <span>90%</span>
                            </div>
                            <div class="review-rate-item">
                                <span>4</span>
                                <span class="review-rate-line" style="--rating: 10%"></span>
                                <span>10%</span>
                            </div>
                            <div class="review-rate-item">
                                <span>3</span>
                                <span class="review-rate-line" style="--rating: 0%"></span>
                                <span>0%</span>
                            </div>
                            <div class="review-rate-item">
                                <span>2</span>
                                <span class="review-rate-line" style="--rating: 0%"></span>
                                <span>0%</span>
                            </div>
                            <div class="review-rate-item">
                                <span>1</span>
                                <span class="review-rate-line" style="--rating: 0%"></span>
                                <span>0%</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="review-comments" id="review-comments">
                    <h5>0 Comments</h5>
                    <form class="review-comments-form">
                        <textarea name="" rows="5" placeholder="Type a Comment here..."></textarea>
                        <div class="review-comments-group">
                            <label class="review-comments-img" for="review-comments-image">
                                <input type="file" name="image" id="review-comments-image" hidden />
                            </label>
                            <input type="text" name="" id="" placeholder="Name*"
                                class="review-comments-input" />
                            <input type="email" name="" id="" placeholder="Email*"
                                class="review-comments-input" />
                            <button type="submit" class="main-btn review-comments-submit">
                                Send
                            </button>
                        </div>
                    </form>
                </div>
            </div>
        </section>
        <!-- END PRODUCT DETAIL REVIEW -->

        <!-- ============================================= -->

        <!-- START PRODUCT DETAIL RECENT -->
        <section class="list list-recent">
            <div class="container">
                <h3 class="margin-57">recently viewed</h3>
                <div class="list-container">
                    <div class="product">
                        <div class="product-img">
                            <img class="product-img-default" src="./assets/image/product-1.png" alt="Culotte dress"
                                title="Culotte dress" />
                            <img class="product-img-hover" src="./assets/image/product-2.png" alt="Culotte dress"
                                title="Culotte dress" />
                            <div class="product-wishlist" data-bs-toggle="offcanvas" data-bs-target="#cart-popup"
                                aria-controls="cart-popup">
                                <svg width="17" height="15" viewBox="0 0 17 15" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M8.44597 14.6433C6.09973 12.9719 3.90258 11.1514 2.10955 8.89149C1.3327 7.91062 0.669627 6.86305 0.343978 5.63107C-0.342632 3.05334 1.32093 0.459919 3.96535 0.0832641C5.778 -0.175686 7.23362 0.507001 8.30081 1.99793C8.35181 2.06855 8.39497 2.13917 8.45774 2.23726C8.5009 2.17448 8.53621 2.12348 8.5676 2.0764C9.56417 0.526619 11.3337 -0.195303 12.9815 0.0871878C14.8609 0.412837 16.238 1.73113 16.6304 3.53201C16.8697 4.64235 16.6578 5.69385 16.187 6.70611C15.6573 7.84784 14.908 8.84048 14.0762 9.77427C12.4362 11.6065 10.5607 13.172 8.55191 14.5805C8.52052 14.5962 8.49305 14.6119 8.44597 14.6433ZM7.88099 4.45403C7.8653 4.31278 7.85353 4.19508 7.83783 4.0813C7.58281 1.71543 4.8756 0.432455 2.86678 1.7272C1.53672 2.58252 1.03843 4.18723 1.61519 5.75662C1.98399 6.76103 2.58821 7.62813 3.26305 8.44421C4.45971 9.8959 5.84863 11.1514 7.31601 12.3206C7.69266 12.6188 8.07717 12.9091 8.45774 13.2034C10.3371 11.7988 12.1027 10.3039 13.5975 8.51091C14.2213 7.76152 14.7824 6.96898 15.1747 6.0705C15.4729 5.38782 15.626 4.68551 15.5082 3.93613C15.2689 2.40989 13.9781 1.28385 12.3812 1.2093C10.9256 1.14261 9.52886 2.18625 9.16397 3.62617C9.09727 3.89297 9.06588 4.17154 9.0188 4.45403C8.64215 4.45403 8.26549 4.45403 7.88099 4.45403Z"
                                        fill="#616161" />
                                </svg>
                            </div>
                            <button class="main-btn main-btn-noborder product-quickview" data-bs-toggle="modal"
                                data-bs-target="#fhm-quickview-popup" aria-controls="fhm-quickview-popup">
                                Quick View
                            </button>
                        </div>
                        <div class="product-info">
                            <span class="product-type">Denim</span>
                            <a href="#" class="product-name" title="Culotte dress">
                                Culotte dress
                            </a>
                            <div class="product-price">
                                <span class="product-price-current">$262.46</span>
                                <span class="product-price-old">$292.99</span>
                            </div>
                        </div>
                        <button class="main-btn add-cart" data-bs-toggle="offcanvas" data-bs-target="#cart-popup"
                            aria-controls="cart-popup">
                            <svg width="16" height="17" viewBox="0 0 16 17" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M1.72117 1.54131C1.33081 1.54131 0.949755 1.54596 0.568694 1.54131C0.159752 1.53666 -0.109779 1.14631 0.0435745 0.77919C0.131869 0.560777 0.303811 0.435306 0.540812 0.430659C1.10776 0.426012 1.67935 0.421365 2.24629 0.430659C2.53906 0.435306 2.75282 0.649072 2.78535 0.946485C2.8597 1.62496 2.92941 2.30808 2.99912 2.98655C3.00841 3.05626 3.0177 3.12596 3.03165 3.21426C3.106 3.21426 3.18035 3.21426 3.2547 3.21426C7.11643 3.21426 10.9781 3.21426 14.8399 3.21426C15.4347 3.21426 15.9041 3.56279 15.9877 4.11579C16.0202 4.33885 15.9831 4.57585 15.9366 4.79891C15.5137 6.79251 15.0908 8.78611 14.654 10.7797C14.4774 11.5976 13.8175 12.1181 12.981 12.1181C10.1184 12.1181 7.25119 12.1181 4.38859 12.1181C3.45918 12.1181 2.77141 11.4768 2.67847 10.5566C2.51582 8.9534 2.33923 7.35016 2.17194 5.75156C2.02788 4.38068 1.88382 3.00979 1.7444 1.63425C1.73511 1.61566 1.73046 1.59243 1.72117 1.54131ZM3.13853 4.3342C3.20823 4.97085 3.27329 5.58427 3.33835 6.20233C3.48706 7.61969 3.63577 9.03705 3.78912 10.4544C3.83094 10.8262 4.03541 11.0074 4.41647 11.0074C7.26513 11.0074 10.1091 11.0074 12.9578 11.0074C13.311 11.0074 13.5062 10.8494 13.5805 10.5009C13.9383 8.83258 14.2962 7.15963 14.654 5.49133C14.7376 5.11027 14.8166 4.72456 14.9003 4.3342C10.9735 4.3342 7.06531 4.3342 3.13853 4.3342Z"
                                    fill="white" />
                                <path
                                    d="M3.34277 14.911C3.34277 13.9909 4.09095 13.238 5.00643 13.238C5.92655 13.238 6.67938 13.9862 6.67938 14.9017C6.67938 15.8218 5.9312 16.5746 5.01572 16.5746C4.10025 16.5793 3.34277 15.8265 3.34277 14.911ZM5.00643 15.4593C5.30849 15.4593 5.56408 15.213 5.56408 14.911C5.56872 14.6089 5.32243 14.3533 5.02037 14.3487C4.71366 14.344 4.45342 14.5996 4.45342 14.9063C4.45807 15.2084 4.70901 15.4593 5.00643 15.4593Z"
                                    fill="white" />
                                <path
                                    d="M11.6893 16.5746C10.7738 16.5746 10.021 15.8218 10.021 14.9017C10.021 13.9862 10.7738 13.2334 11.6939 13.2334C12.6141 13.2334 13.3622 13.9862 13.3622 14.9063C13.3622 15.8265 12.6094 16.5746 11.6893 16.5746ZM11.6893 14.3487C11.3872 14.3487 11.1363 14.5996 11.1363 14.9017C11.1363 15.2038 11.3826 15.4547 11.6846 15.4593C11.9914 15.464 12.2516 15.2038 12.2469 14.8971C12.2469 14.595 11.9914 14.3487 11.6893 14.3487Z"
                                    fill="white" />
                            </svg>

                            Add to Cart
                        </button>
                    </div>
                    <div class="product">
                        <div class="product-img">
                            <img class="product-img-default" src="./assets/image/product-7.png" alt="Culotte dress"
                                title="Culotte dress" />
                            <img class="product-img-hover" src="./assets/image/product-3.png" alt="Culotte dress"
                                title="Culotte dress" />
                            <div class="product-wishlist" data-bs-toggle="offcanvas" data-bs-target="#cart-popup"
                                aria-controls="cart-popup">
                                <svg width="17" height="15" viewBox="0 0 17 15" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M8.44597 14.6433C6.09973 12.9719 3.90258 11.1514 2.10955 8.89149C1.3327 7.91062 0.669627 6.86305 0.343978 5.63107C-0.342632 3.05334 1.32093 0.459919 3.96535 0.0832641C5.778 -0.175686 7.23362 0.507001 8.30081 1.99793C8.35181 2.06855 8.39497 2.13917 8.45774 2.23726C8.5009 2.17448 8.53621 2.12348 8.5676 2.0764C9.56417 0.526619 11.3337 -0.195303 12.9815 0.0871878C14.8609 0.412837 16.238 1.73113 16.6304 3.53201C16.8697 4.64235 16.6578 5.69385 16.187 6.70611C15.6573 7.84784 14.908 8.84048 14.0762 9.77427C12.4362 11.6065 10.5607 13.172 8.55191 14.5805C8.52052 14.5962 8.49305 14.6119 8.44597 14.6433ZM7.88099 4.45403C7.8653 4.31278 7.85353 4.19508 7.83783 4.0813C7.58281 1.71543 4.8756 0.432455 2.86678 1.7272C1.53672 2.58252 1.03843 4.18723 1.61519 5.75662C1.98399 6.76103 2.58821 7.62813 3.26305 8.44421C4.45971 9.8959 5.84863 11.1514 7.31601 12.3206C7.69266 12.6188 8.07717 12.9091 8.45774 13.2034C10.3371 11.7988 12.1027 10.3039 13.5975 8.51091C14.2213 7.76152 14.7824 6.96898 15.1747 6.0705C15.4729 5.38782 15.626 4.68551 15.5082 3.93613C15.2689 2.40989 13.9781 1.28385 12.3812 1.2093C10.9256 1.14261 9.52886 2.18625 9.16397 3.62617C9.09727 3.89297 9.06588 4.17154 9.0188 4.45403C8.64215 4.45403 8.26549 4.45403 7.88099 4.45403Z"
                                        fill="#616161" />
                                </svg>
                            </div>
                            <button class="main-btn main-btn-noborder product-quickview" data-bs-toggle="modal"
                                data-bs-target="#fhm-quickview-popup" aria-controls="fhm-quickview-popup">
                                Quick View
                            </button>
                        </div>
                        <div class="product-info">
                            <span class="product-type">Dress</span>
                            <a href="#" class="product-name" title="Culotte dress">
                                Wrap around dress
                            </a>
                            <div class="product-price">
                                <span class="product-price-current">$134.46</span>
                                <span class="product-price-old">$292.99</span>
                            </div>
                        </div>
                        <button class="main-btn add-cart" data-bs-toggle="offcanvas" data-bs-target="#cart-popup"
                            aria-controls="cart-popup">
                            <svg width="16" height="17" viewBox="0 0 16 17" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M1.72117 1.54131C1.33081 1.54131 0.949755 1.54596 0.568694 1.54131C0.159752 1.53666 -0.109779 1.14631 0.0435745 0.77919C0.131869 0.560777 0.303811 0.435306 0.540812 0.430659C1.10776 0.426012 1.67935 0.421365 2.24629 0.430659C2.53906 0.435306 2.75282 0.649072 2.78535 0.946485C2.8597 1.62496 2.92941 2.30808 2.99912 2.98655C3.00841 3.05626 3.0177 3.12596 3.03165 3.21426C3.106 3.21426 3.18035 3.21426 3.2547 3.21426C7.11643 3.21426 10.9781 3.21426 14.8399 3.21426C15.4347 3.21426 15.9041 3.56279 15.9877 4.11579C16.0202 4.33885 15.9831 4.57585 15.9366 4.79891C15.5137 6.79251 15.0908 8.78611 14.654 10.7797C14.4774 11.5976 13.8175 12.1181 12.981 12.1181C10.1184 12.1181 7.25119 12.1181 4.38859 12.1181C3.45918 12.1181 2.77141 11.4768 2.67847 10.5566C2.51582 8.9534 2.33923 7.35016 2.17194 5.75156C2.02788 4.38068 1.88382 3.00979 1.7444 1.63425C1.73511 1.61566 1.73046 1.59243 1.72117 1.54131ZM3.13853 4.3342C3.20823 4.97085 3.27329 5.58427 3.33835 6.20233C3.48706 7.61969 3.63577 9.03705 3.78912 10.4544C3.83094 10.8262 4.03541 11.0074 4.41647 11.0074C7.26513 11.0074 10.1091 11.0074 12.9578 11.0074C13.311 11.0074 13.5062 10.8494 13.5805 10.5009C13.9383 8.83258 14.2962 7.15963 14.654 5.49133C14.7376 5.11027 14.8166 4.72456 14.9003 4.3342C10.9735 4.3342 7.06531 4.3342 3.13853 4.3342Z"
                                    fill="white" />
                                <path
                                    d="M3.34277 14.911C3.34277 13.9909 4.09095 13.238 5.00643 13.238C5.92655 13.238 6.67938 13.9862 6.67938 14.9017C6.67938 15.8218 5.9312 16.5746 5.01572 16.5746C4.10025 16.5793 3.34277 15.8265 3.34277 14.911ZM5.00643 15.4593C5.30849 15.4593 5.56408 15.213 5.56408 14.911C5.56872 14.6089 5.32243 14.3533 5.02037 14.3487C4.71366 14.344 4.45342 14.5996 4.45342 14.9063C4.45807 15.2084 4.70901 15.4593 5.00643 15.4593Z"
                                    fill="white" />
                                <path
                                    d="M11.6893 16.5746C10.7738 16.5746 10.021 15.8218 10.021 14.9017C10.021 13.9862 10.7738 13.2334 11.6939 13.2334C12.6141 13.2334 13.3622 13.9862 13.3622 14.9063C13.3622 15.8265 12.6094 16.5746 11.6893 16.5746ZM11.6893 14.3487C11.3872 14.3487 11.1363 14.5996 11.1363 14.9017C11.1363 15.2038 11.3826 15.4547 11.6846 15.4593C11.9914 15.464 12.2516 15.2038 12.2469 14.8971C12.2469 14.595 11.9914 14.3487 11.6893 14.3487Z"
                                    fill="white" />
                            </svg>

                            Add to Cart
                        </button>
                    </div>
                </div>
            </div>
        </section>
        <!-- END PRODUCT DETAIL RECENT -->

    </main>
    <style>
        #fhm-product-detail #banner-container .banner-category .megamenu-wrapper {
            position: relative;
        }

        #fhm-product-detail #banner-container .banner-category .megamenu-wrapper .megamenu-body {
            display: none;
            position: absolute;
            top: 100%;
            left: 0;
            z-index: 999;
            background: #fff;
            width: 100%;
            max-height: unset;
            overflow: unset;
        }

        #fhm-product-detail .menu-banner .slider-banner {
            display: none;
        }

        #fhm-product-detail #banner-container .banner-category .megamenu-wrapper:hover .megamenu-body {
            display: block;
        }
    </style>
    <script>
        $(document).ready(function() {
            $('#zoomImage').zoomLens({
                lensSize: 200 // Kích thước của lens, có thể điều chỉnh theo ý muốn
            });
        });
    </script>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('frontend.layouts.default', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH F:\NEWWAY\Nw_Quatang\resources\views/frontend/pages/product/detail.blade.php ENDPATH**/ ?>