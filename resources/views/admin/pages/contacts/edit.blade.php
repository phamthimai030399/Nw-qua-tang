                ...
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <select name="json_params[department_id]" id="department_id" class="form-control select2"
                        style="width:100%">
                        <option value="">@lang('Please select')</option>
                        @foreach ($departments as $item)
                          @if ($item->parent_id == 0 || $item->parent_id == null)
                            <option value="{{ $item->id }}"
                              {{ isset($detail->json_params->department_id) && $detail->json_params->department_id == $item->id ? 'selected' : '' }}>
                              {{ $item->title }}</option>

                            @foreach ($departments as $sub)
                              @if ($item->id == $sub->parent_id)
                                <option value="{{ $sub->id }}"
                                  {{ isset($detail->json_params->department_id) && $detail->json_params->department_id == $sub->id ? 'selected' : '' }}>
                                  - - {{ $sub->title }}
                                </option>
                                ...
                                @foreach ($departments as $sub_child)
                                  @if ($sub->id == $sub_child->parent_id)
                                    <option value="{{ $sub_child->id }}"
                                      {{ isset($detail->json_params->department_id) && $detail->json_params->department_id == $sub_child->id ? 'selected' : '' }}>
                                      - - - -
