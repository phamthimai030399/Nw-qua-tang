...
<ul class="nav nav-pills">
	...
	<li class="active"><a data-toggle="pill" href="#category_{{ $taxonomy->id }}">{{ $taxonomy->title }}</a></li>
	<?php 
	$i=0;
	foreach($taxonomy_all as $sub_taxonomy){ 
	  if($sub_taxonomy->parent_id == $taxonomy->id){ $i++; array_push($array_category,$sub_taxonomy->id );
	?>
	<li><a data-toggle="pill" href="#category_{{ $sub_taxonomy->id }}">{{ $sub_taxonomy->title }}</a></li>
	<?php }} ?>
</ul>

<div class="tab-content">
	
	<div id="category_{{ $taxonomy->id }}" class="tab-pane fade in active">
	
		<div class="row">
			<div class="col-md-3 display-pc">
				<div class="tab-images">
					<a href="/{{ $taxonomy->taxonomy }}/{{ $taxonomy->url_part }}.html">
						<img lazy="" src="{{ $taxonomy->json_params->image }}" data-original="{{ $taxonomy->json_params->image }}" alt="" width="312" height="560"></a>
				</div>
			</div>
			<div class="col-md-9">
	...
	foreach($taxonomy_all as $sub_taxonomy){ 
	  if($sub_taxonomy->parent_id == $taxonomy->id){
		$key2=0;
	?>
  <div id="category_{{ $sub_taxonomy->id }}" class="tab-pane fade in">
					...
					<div class="block col-sm-3 col-xs-6 col-mobile-12  ">
						...
						<div id="idpr_5" class="product product_wg clearfix product-hover">
							...
							<div class="right">
								...
								<div class="shopping-cart" style="width: 20%; float: left; cursor: pointer;" onclick="addToCart({{ $spnoibat->id }})">
									<i class="fa-solid fa-shopping-cart"></i>
								</div>
